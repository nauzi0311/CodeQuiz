...
class QuestionTypePageArgs {
  List<String> uans = [], ans = [], exp = [];
  List<Question> quest = [];
  QuestionTypePageArgs(this.uans, this.ans, this.quest, this.exp);
}

class QuestionTypePage extends StatefulWidget {
  const QuestionTypePage({Key? key}) : super(key: key);
...
class _QuestionTypePageState extends State<QuestionTypePage> {
          ...
          if (_controller.text == answer) {
            if (pcount >= 7) {
              ...
              Navigator.of(context).pushNamed('/questiontype',
                  arguments: QuestionTypePageArgs(uans, ans, quest, exp));
            }
          } else {
  ...
  Widget build(BuildContext context) {
    final args =
        ModalRoute.of(context)!.settings.arguments as QuestionTypePageArgs;
    uans = args.uans;
    ans = args.ans;
    quest = args.quest;
    exp = args.exp;
